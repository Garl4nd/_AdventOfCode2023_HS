              ...
              let newBeams =  foldr updateLists [] movedBeams
                  updateLists beam acc   
                         ...
                         | otherwise = (reflect beam currentTile) :acc where
                          ...
                          currentTile = traceWInfo "currentTile" $ tileGrid ! (y,x)
                          (y, x) =  pos $ traceWInfo "beam" beam
...
updateST tileGrid st = do
  ...
  stAr <- fst <$> st
  forM_ updatedBeams $ \Beam {pos = (y,x), dir = bdir} -> when (inBounds bounds y x) $  modifyArray stAr (y,x) (combineD bdir) 
...
updateST' tileGrid st = do
  (stAr, beams) <- st   
  forM_ beams $ \Beam {pos = bpos, dir = bdir} -> modifyArray stAr bpos (combineD bdir)               
  updatedBeams <- updateBeamSeqST tileGrid st
  return (stAr, updatedBeams) 
...
simulationR  = do
  ...
  let sim st' = do 
        ...
        if null beams then return (ar, []) else sim (return  (ar, beams)) 
  lift $ put (sim st)
...
countVisits file initBeam = countIf (/=0) $ A.toList  (gameResR (parseFile file) initBeam)

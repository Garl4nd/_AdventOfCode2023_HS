...
aStarLoop :: forall node graph s. (Show node, LabeledGraph graph node) => graph -> (node -> NumType) -> ST s (DijkstraStateST node s) ->  ST s (DijkstraStateST node s)
aStarLoop graph heuristics dsST = do
    ds@DijkstraStateST {finalStatesST = fs, distanceMapST = dm, nodeQueueST = nq, destsST = dsts}  <- dsST    
    if S.null $ dsts  then 
        dsST -- return ds
    else
        case H.view nq of 
            ...
            Just (distNode, nq')  ->   processNode distNode where
                processNode :: (Distance, node) ->  ST s (DijkstraStateST node s)
                processNode (_, minNode) 
                    |  S.member minNode fs =  aStarLoop graph heuristics (return $ ds {nodeQueueST = nq'}  )                                 
                processNode (_, minNode) = do                                                             
                    minNodeDist <- readArray dm minNode
                    let candidateList = [ (neighbor, newDist) | (neighbor, edgeVal) <- getEdges graph  minNode, S.notMember neighbor fs,  let newDist =  addDist minNodeDist edgeVal]
                    currentDists <- forM candidateList (\(neigh, _) -> readArray dm neigh)
                    let updateList = [ (neighbor, newDist) | ((neighbor, newDist), currentDist) <- zip candidateList currentDists, newDist <= currentDist  ] 
                        updatedQueue = foldl' (\q (node, dist')    ->  H.insert (addDist dist' $ heuristics  node, node) q  ) nq' updateList             
                    forM_ updateList $ \(pos, newDist) -> writeArray dm pos newDist                    
                    aStarLoop graph heuristics  (return $ ds {finalStatesST = S.insert  minNode fs, distanceMapST = dm, nodeQueueST = updatedQueue, destsST = S.delete minNode dsts } )
...
runAstarST ::  forall graph node. (Show node, LabeledGraph graph node) => graph -> (node-> NumType) -> node -> [node]  -> DijkstraState node
runAstarST graph heuristics start ends =  extractFromST $ aStarLoop graph heuristics initState  where
    extractFromST :: (forall s. ST s (DijkstraStateST node s)) -> DijkstraState node
    extractFromST stRes = DijkstraState {finalStates = fs, distanceMap = dm, nodeQueue = nq, dests = dst } where
        dm = runSTArray $ do  
            DijkstraStateST { distanceMapST = dmST } <- stRes -- runDijkstraST graph start ends
    ...
    initState =  do 
                            stAr <- newArray (getBounds graph ) Inf
                            writeArray stAr start  $ Dist 0
                            return DijkstraStateST {finalStatesST = S.empty, distanceMapST = stAr, nodeQueueST = H.singleton ( Dist 0, start), destsST = S.fromList ends, targetNode = head ends } -- runDijkstraSTLow graph start ends

solveWAStarST :: (Show node, LabeledGraph graph node) => graph -> (node-> NumType)-> node  -> [node] -> DistanceMap node
solveWAStarST graph heuristics start ends = distanceMap $ runAstarST graph heuristics start ends


taxiCab :: AugPos -> AugPos -> NumType
taxiCab ((yT, xT), _) ((y,x), _) = abs (yT-y) + (xT- x) 
...
solveAugmentedGraph graph start end = let initStates = [(start, dir) | dir <- [H, V] ]
                                          ...
                                          let distMap =  solveWAStarST graph (taxiCab $ head endStates) initState endStates in map (distMap !) endStates | initState <- initStates]
...
numGridToStr charGrid = let rows = [[charGrid ! (y,x) | x <- [xmin..xmax] ]| y<- [ymin..ymax]]
                            ...
                            show <$> rows   
getSolutions17Astar :: String -> IO (Int, Int)
getSolutions17Astar filename =  do 
    file <- readFile filename
    let ar = strToAR file
        graph = nodeMapToAugmentedGraph  ar minMoves1 maxMoves1
        graph2 = nodeMapToAugmentedGraph  ar minMoves2 maxMoves2
        ordGraph = nodeMapToGraph ar         
        ((startPos, _), (endPos@(yT,xT), _)) = A.bounds graph
        -- distMap = 

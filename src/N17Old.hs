    ...
    Just (distNode, nq')  ->   processNode distNode where
        ...
        processNode (dist, minNode) = updateDS graph $ ds {finalStates = S.insert  minNode fs, distanceMap = updatedDistanceMap, nodeQueue = updatedQueue, dests = S.delete minNode dsts }  where
            updatedDistanceMap = if null updateList then dm else  dm // updateList -- [(--updateList 
                --  runSTArray $ do
                --      stAr <- unsafeThawSTArray dm  -- // updateList 
                --      forM_ updateList $ \(pos, newDist) -> writeArray stAr pos newDist
                --      return stAr
            updateList = [ (neighbor, newDist) | (neighbor, edgeVal) <- getEdges graph  minNode, S.notMember neighbor fs,  let newDist = addDist dist  edgeVal, newDist <= dm ! neighbor  ] 
...
runDijkstra graph start ends =  traceWInfo2 "" "Running the unsafe normal version" $ updateDS graph initState where
    initState = DijkstraState {finalStates = S.empty, distanceMap = initDistanceMap, nodeQueue = H.singleton ( Dist 0, start), dests = S.fromList ends }
    initDistanceMap = runSTArray $ do 
...
runDijkstraSTLow graph start ends =  (updateDSST graph initState) where
    initState = do 
        stAr <- newArray (A.bounds graph ) Inf
        writeArray stAr start  $ Dist 0
        return DijkstraStateST {finalStatesST = S.empty, distanceMapST = stAr, nodeQueueST = H.singleton ( Dist 0, start), destsST = S.fromList ends }
...
solveAugmentedGraph graph start end minMoves maxMoves = let initStates = [(start, dir, 0) | dir <- [R, D] ]
...
calcPos p d  = calcPosWStep p d 1

redDirToDirs :: RedDir -> [Dir]
redDirToDirs H = [L, R]
redDirToDirs V = [U, D]

dirToRedDir :: Dir -> RedDir
dirToRedDir L =  H
dirToRedDir R =  H
dirToRedDir D =  V
dirToRedDir U =  V
...
genAugEdges (pos, dir, moves) ((ymin, xmin), (ymax, xmax)) nodeAr minMoves maxMoves =  -- if moves < minMoves then 
    ...
    --else
     ...
     liftedEdges = do                 
        ...
        --dir' <- if moves < minMoves then [dir] else    [H,V]
        
        
        let moves' = if dir == dir' then moves+1 else 1           
            pos' = calcPos pos dir'
        -- pos' <- calcPos pos <$>  redDirToDirs dir'
        guard (moves' <=maxMoves &&  inBounds pos')
        return ((pos', dir', moves'), nodeAr ! pos')                   
     inBounds (y,x) = ymin <= y && y <= ymax && xmin <= x && x <= xmax

gridGraph :: LabeledGraph Pos2D
gridGraph = A.listArray bounds listVals where
    bounds = ((1,1),(10,10))
    coords = concat [[(y,x) |x <- [1..10]] | y <- [1..10]]
    listVals = [genEdges coord bounds 1 | coord <- coords]

constAr :: A.Array Pos2D NumType
constAr = runSTArray $ do {ar <- newArray ((1,1),(10,10)) 1; writeArray ar (1,2) 10 ; return ar}

modifiedGrid  = runSTArray $ do 
    stAr <- thaw gridGraph
    writeArray stAr (1,1) [((1,2), 100), ((2,1), 15)]
    writeArray stAr (9,10) [((10,10), 100)]
    writeArray stAr (10,9) [((10,10), 100)]
    return stAr
...
nodeMapToAugmentedGraph nodeMap minMoves maxMoves = A.listArray augBounds listVals where
    ...
    --listVals = [ traceWInfo ("node/edge= " <> show   augCoord) $ genAugEdges augCoord bounds nodeMap minMoves maxMoves | augCoord <- augCoords]
...
solveSimple filename =  do 
    ...
    return ( fromDist $ (distanceMap ds ) ! endPos, 0)
    --print $ solveWDijkstra graph (startPos, R, 3) (endPos, R, 2)

getSolutions17Old :: String -> IO (Int, Int)
getSolutions17Old filename =  do 
    file <- readFile filename
    let ar = strToAR file
        ...
        solution1 =  fromDist $  solveAugmentedGraph graph startPos endPos 1 maxMoves1          
        solution2 =  fromDist $ solveAugmentedGraph graph2 startPos endPos minMoves2 maxMoves2
